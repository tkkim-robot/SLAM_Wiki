...
public:
  ...
  void onInit() override {
    ...
    // get topic name : KTK
    std::string _points_topic = private_nh.param<std::string>("points_topic", "/velodyne_points");
    std::string _imu_topic = private_nh.param<std::string>("imu_topic", "/gpsimu_driver/imu_data");
    std::string _gps_topic = private_nh.param<std::string>("gps_topic", "/gpsimu_driver/nmea_sentence");
    ...
    points_sub = mt_nh.subscribe(_points_topic, 5, &HdlLocalizationNodelet::points_callback, this);
    globalmap_sub = nh.subscribe(_imu_topic, 1, &HdlLocalizationNodelet::globalmap_callback, this);
    initialpose_sub = nh.subscribe(_gps_topic, 8, &HdlLocalizationNodelet::initialpose_callback, this);
